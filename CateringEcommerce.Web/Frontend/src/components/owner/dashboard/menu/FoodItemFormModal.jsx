...
export default function FoodItemFormModal({ isOpen, onClose, onSave, editingItem, categories, cuisines }) {
    const getInitialState = () => ({
        name: '', description: '', categoryId: '', typeId: 0 , price: '', isPackageItem: false, status: true, media: []
    });
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value={formData.name} onChange={handleChange} autoComplete="off" className={`mt-1 block w-full px-3 py-2 border ${errors.name ? 'border-red-500' : 'border-neutral-300'} rounded-md shadow-sm`} />
                    ...
                    {/*    media={media}*/}
                    {/*    setMedia={setMedia}*/}
                    {/*    error={media.length === 0 ? "Please upload at least one media file" : ""}*/}
                    {/*/>*/}
                    ...
                    <div className="space-y-3 pt-2">
                        ...
                        <ToggleSwitch label="Status" enabled={formData.status} setEnabled={() => handleToggle('status')} />
