...
export default function ProfileSettings() {
    ...
    const handleUpdate = async (section, data) => {
        ...
        try {
            ...
            alert(`Error: ${error.message || 'Could not update details.'}`);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            case TABS.BUSINESS:
                return <BusinessAccountSettings initialData={profileData.formData.ownerBusiness} onUpdate={(data) => handleUpdate('business', data)} />;
            case TABS.ADDRESS:
                return <AddressSettings initialData={profileData.formData.cateringAddress} onUpdate={(data) => handleUpdate('address', data)} />;
            case TABS.SERVICES:
                return <ServicesSettings initialData={profileData.formData.cateringServices} onUpdate={(data) => handleUpdate('services', data)} />;
            case TABS.LEGAL:
                return <LegalPaymentSettings initialData={profileData.formData.ownerLegalDocument} onUpdate={(data) => handleUpdate('legal', data)} />;
            default:
                return null;
        }
    };
