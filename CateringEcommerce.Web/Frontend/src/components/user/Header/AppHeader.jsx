                    ...
                    <div className="flex items-center">
                        <div className="flex-shrink-0">
                            <Link to="/" className="text-3xl font-bold text-rose-600 flex items-center">
                                <span className="icon-placeholder text-3xl mr-1">🍽️</span> Feasto
                            </Link>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        {isAuthenticated ? (
                            <div className="relative" ref={dropdownRef}>
                                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center text-sm font-medium text-neutral-600 hover:text-rose-600 focus:outline-none">
                                ...
                                {isDropdownOpen && (
                                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-20">
                                        ...
                                        <Link to="/profile" onClick={() => setIsDropdownOpen(false)} className="block w-full text-left px-4 py-2 text-sm text-neutral-700 hover:bg-amber-100 hover:text-rose-600">My Profile</Link>
                                        <a href="#" className="block px-4 py-2 text-sm text-neutral-700 hover:bg-amber-100 hover:text-rose-600">My Orders</a>
                                        <button onClick={() => { handleLogout(); setIsDropdownOpen(false); }} className="block w-full text-left px-4 py-2 text-sm text-neutral-700 hover:bg-amber-100 hover:text-rose-600">Logout</button>
                                    </div>
                                )}
                            </div>
                        ) : (
