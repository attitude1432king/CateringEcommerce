...
namespace CateringEcommerce.BAL.Common.Admin
{
    public class AdminDashboardRepository : IAdminDashboardRepository
    {
        ...
        private (int Total, int Active, int Pending, int Today, int ThisMonth) GetCateringMetrics()
        {
            string query = $@"
                SELECT
                    ...
                    COUNT(CASE WHEN c_status = 'Pending' THEN 1 END) AS Pending,
                    COUNT(CASE WHEN CAST(c_created_date AS DATE) = CAST(GETDATE() AS DATE) THEN 1 END) AS Today,
                    COUNT(CASE WHEN MONTH(c_created_date) = MONTH(GETDATE()) AND YEAR(c_created_date) = YEAR(GETDATE()) THEN 1 END) AS ThisMonth
        ...
        private (int Total, int Pending, int Completed, int Cancelled, int Today, int ThisMonth) GetOrderMetrics()
        {
            string query = $@"
                SELECT
                    COUNT(*) AS Total,
                    COUNT(CASE WHEN c_status = 'Pending' THEN 1 END) AS Pending,
                    COUNT(CASE WHEN c_status = 'Completed' THEN 1 END) AS Completed,
                    COUNT(CASE WHEN c_status = 'Cancelled' THEN 1 END) AS Cancelled,
                    COUNT(CASE WHEN CAST(c_order_date AS DATE) = CAST(GETDATE() AS DATE) THEN 1 END) AS Today,
                    COUNT(CASE WHEN MONTH(c_order_date) = MONTH(GETDATE()) AND YEAR(c_order_date) = YEAR(GETDATE()) THEN 1 END) AS ThisMonth
        ...
        private (decimal Total, decimal Today, decimal ThisMonth, decimal Commission, decimal AvgOrderValue) GetRevenueMetrics()
        {
            string query = $@"
                SELECT
                    ISNULL(SUM(CASE WHEN c_status = 'Completed' THEN c_total_amount ELSE 0 END), 0) AS Total,
                    ISNULL(SUM(CASE WHEN c_status = 'Completed' AND CAST(c_order_date AS DATE) = CAST(GETDATE() AS DATE) THEN c_total_amount ELSE 0 END), 0) AS Today,
                    ISNULL(SUM(CASE WHEN c_status = 'Completed' AND MONTH(c_order_date) = MONTH(GETDATE()) AND YEAR(c_order_date) = YEAR(GETDATE()) THEN c_total_amount ELSE 0 END), 0) AS ThisMonth,
                    ISNULL(SUM(CASE WHEN c_status = 'Completed' THEN c_platform_commission ELSE 0 END), 0) AS Commission,
                    CASE WHEN COUNT(CASE WHEN c_status = 'Completed' THEN 1 END) > 0
                         THEN ISNULL(SUM(CASE WHEN c_status = 'Completed' THEN c_total_amount ELSE 0 END), 0) / COUNT(CASE WHEN c_status = 'Completed' THEN 1 END)
        ...
        private (decimal AvgRating, int Total, int ThisMonth) GetReviewMetrics()
        {
            string query = $@"
                SELECT
                    ISNULL(AVG(CAST(c_rating AS DECIMAL(3,2))), 0) AS AvgRating,
                    COUNT(*) AS Total,
                    COUNT(CASE WHEN MONTH(c_created_date) = MONTH(GETDATE()) AND YEAR(c_created_date) = YEAR(GETDATE()) THEN 1 END) AS ThisMonth
        ...
        private List<AdminTopCatering> GetTopCaterings()
        {
            string query = $@"
                SELECT TOP 5
                    ...
                    ISNULL(AVG(CAST(r.c_rating AS DECIMAL(3,2))), 0) AS Rating
                ...
                LEFT JOIN {Table.SysCateringReview} r ON co.c_catering_ownerid = r.c_catering_ownerid
                LEFT JOIN {Table.City} c ON co.c_cityid = c.c_cityid
                WHERE co.c_isactive = 1
                GROUP BY co.c_catering_ownerid, co.c_business_name, c.c_cityname
        ...
        private List<AdminRecentOrder> GetRecentOrders()
        {
            string query = $@"
                SELECT TOP 10
                    ...
                    o.c_status AS Status,
                    o.c_order_date AS OrderDate,
        ...
        private List<AdminRevenueChart> GetRevenueChart()
        {
            string query = $@"
                SELECT
                    CAST(o.c_order_date AS DATE) AS Date,
                ...
                FROM {Table.SysOrders} o
                WHERE o.c_status = 'Completed'
                  AND o.c_order_date >= DATEADD(DAY, -7, GETDATE())
                GROUP BY CAST(o.c_order_date AS DATE)
                ORDER BY Date";
