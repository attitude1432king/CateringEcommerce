...
namespace CateringEcommerce.BAL.Common.Admin
{
    public class RBACRepository : IRBACRepository
    {
        ...
        public async Task<RoleDetailResponse?> GetRoleByIdAsync(long roleId)
        {
            ...
            var adminQuery = @"
                SELECT
                    a.c_admin_id, a.c_full_name, a.c_email, a.c_is_active, a.c_created_date
                FROM t_sys_admin_users a
                INNER JOIN t_sys_admin_user_roles ur ON a.c_admin_id = ur.c_admin_id
            ...
            foreach (DataRow adminRow in adminDt.Rows)
            {
                ...
                {
                    AdminId = Convert.ToInt64(adminRow["c_admin_id"]),
        ...
        public async Task<List<string>> GetAdminPermissionsAsync(long adminId)
        {
            var query = @"
                ...
                WHERE ur.c_admin_id = @AdminId AND ur.c_is_active = 1 AND p.c_is_active = 1";
        ...
        public async Task<List<string>> GetAdminRolesAsync(long adminId)
        {
            var query = @"
                ...
                WHERE ur.c_admin_id = @AdminId AND ur.c_is_active = 1 AND r.c_is_active = 1";
        ...
        public async Task<bool> AssignRolesToAdminAsync(long adminId, List<long> roleIds, long assignedBy)
        {
            ...
            var deactivateQuery = "UPDATE t_sys_admin_user_roles SET c_is_active = 0 WHERE c_admin_id = @AdminId";
            ...
            {
                var upsertQuery = @"
                    IF EXISTS (SELECT 1 FROM t_sys_admin_user_roles WHERE c_admin_id = @AdminId AND c_role_id = @RoleId)
                        UPDATE t_sys_admin_user_roles SET c_is_active = 1 WHERE c_admin_id = @AdminId AND c_role_id = @RoleId
                    ELSE
                        INSERT INTO t_sys_admin_user_roles (c_admin_id, c_role_id, c_assigned_by)
        ...
        public async Task<bool> RemoveRoleFromAdminAsync(long adminId, long roleId)
        {
            var query = @"
                ...
                SET c_is_active = 0
                WHERE c_admin_id = @AdminId AND c_role_id = @RoleId";
        ...
        public async Task LogAuditAsync(AuditLogEntry entry)
        {
            var query = @"
                ...
                (c_admin_id, c_admin_name, c_action, c_module, c_target_id, c_target_type,
        ...
        public async Task<AuditLogListResponse> GetAuditLogsAsync(AuditLogListRequest request)
        {
            ...
            {
                conditions.Add("c_admin_id = @AdminId");
            ...
            var dataQuery = $@"
                SELECT
                    c_audit_id, c_admin_id, c_admin_name, c_action, c_module,
            ...
            foreach (DataRow row in dt.Rows)
            {
                logs.Add(new AuditLogEntry
                {
                    ...
                    AdminId = Convert.ToInt64(row["c_admin_id"]),
