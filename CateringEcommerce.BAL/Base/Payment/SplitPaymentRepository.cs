...
namespace CateringEcommerce.BAL.Base.Payment
{
    public class SplitPaymentRepository : ISplitPaymentRepository
    {
        ...
        {
            var query = @"
                SELECT * FROM t_sys_payment_transactions
        ...
        {
            var query = @"
                SELECT * FROM t_sys_escrow_ledger
                WHERE c_orderid = @OrderId
                ORDER BY c_createddate DESC";
        ...
        public async Task<List<PartnerPayoutRequest>> GetPartnerPayoutRequestsAsync(long cateringOwnerId)
        {
            var query = @"
                ...
                FROM t_sys_vendor_payout_requests vpr
                INNER JOIN t_sys_order o ON vpr.c_orderid = o.c_orderid
                INNER JOIN t_sys_catering_owner co ON vpr.c_cateringownerid = co.c_ownerid
        ...
        public async Task<PaymentDashboard> GetPaymentDashboardAsync()
        {
            var query = @"
                SELECT
                    ...
                    ISNULL((SELECT SUM(c_escrowamount) FROM t_sys_order_payment_summary WHERE c_escrowstatus = 'HELD'), 0) AS EscrowBalance,
                    (SELECT COUNT(*) FROM t_sys_order_payment_summary WHERE c_advancepaid = 1 AND c_vendoradvancereleased = 0) AS PendingAdvanceReleases,
                    (SELECT COUNT(*) FROM t_sys_order_payment_summary WHERE c_finalpaid = 1 AND c_vendorpayoutstatus <> 'COMPLETED') AS PendingFinalPayouts
            ...
            var txnQuery = @"
                SELECT TOP 10 * FROM t_sys_payment_transactions
                WHERE c_paymentstatus = 'SUCCESS'
        ...
        public async Task<EscrowDashboard> GetEscrowDashboardAsync()
        {
            var query = @"
                SELECT
                    ...
                    (SELECT COUNT(*) FROM t_sys_order_payment_summary WHERE c_advancepaid = 1 AND c_vendoradvancereleased = 0) AS PendingReleases,
                    ISNULL((SELECT SUM(c_advanceamount) FROM t_sys_order_payment_summary WHERE c_advancepaid = 1 AND c_vendoradvancereleased = 0), 0) AS PendingReleaseAmount,
                    (SELECT COUNT(*) FROM t_sys_escrow_ledger WHERE CAST(c_createddate AS DATE) = CAST(GETDATE() AS DATE) AND c_status = 'COMPLETED') AS CompletedReleasesToday,
                    ISNULL((SELECT SUM(c_amount) FROM t_sys_escrow_ledger WHERE CAST(c_createddate AS DATE) = CAST(GETDATE() AS DATE) AND c_status = 'COMPLETED'), 0) AS CompletedReleaseAmountToday
            ...
            // Get recent transactions
            var txnQuery = @"
                SELECT TOP 20 * FROM t_sys_escrow_ledger
                ORDER BY c_createddate DESC";
        ...
        {
            var query = @"
                SELECT * FROM t_sys_payment_gateway_config
